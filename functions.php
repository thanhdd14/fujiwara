...
/**
 * ACFプラグインがインストール＆アクティベートされているかチェックする関数
 */
function check_for_acf_plugin() {
    // ACFプラグインがアクティブかどうかをチェック
    if ( ! class_exists( 'ACF' ) ) {
        // 管理画面に警告を表示
...
/**
 * ACFがインストールされていない場合に管理画面に表示する警告
 */
function acf_not_installed_notice() {
    ?>
    <div class="notice notice-warning">
        <p><?php _e( 'このテーマを使用するには、Advanced Custom Fieldsプラグインをインストールしてアクティベートする必要があります。', 'your-text-domain' ); ?></p>
    </div>
    <?php
}
...
// OC MENU WALKER
require_once('inc/functions-oc-menu-walker.php');
// OC SERVICE PACK
require_once('inc/functions-oc-service-pack-cpt.php');
require_once('inc/functions-oc-service-pack-acf.php');
// OC PRICE PACK
require_once('inc/functions-oc-price-acf.php');
// END of OC CTA PACK
